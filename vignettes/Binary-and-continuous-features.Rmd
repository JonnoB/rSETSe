...
The two_bicomponents is a simple network constructed of two groups of nodes that are linked by a single edge. This means the network is made of 3 bi-connected components hence the name. For more information type `?two_bicomponents`
```{r message=FALSE}
two_bicomponents %>%
  ggraph() + 
  geom_edge_link() +
...
```{r message=FALSE}
embeddings_cont <- two_bicomponents %>%
...
embeddings_cont_fixed <- two_bicomponents %>%
...
embeddings_binary <- two_bicomponents %>%
  prepare_SETSe_binary(., node_names = "name", force_var = "group", positive_value = "A") %>%
  SETSe_auto(., k = "weight")


embeddings_binary_fixed <- two_bicomponents %>%
