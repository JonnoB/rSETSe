...
#' Setting this value to be 0.1% of the initial static force is usually a good idea
...
#' @param hyper_tol numeric. The convergence tolerance when trying to find the minimum value. When the ratio between the current static
#' force and the previous static force is smaller than this value the search terminates.
#' values between 0.1-0.3 seem to be often ok, too small and you waste time fine tuning the parameters instead of converging
#' to big and you have poorly parametrised values.
...
setse_auto_hd <- function(g,
  ...
  if(verbose){print("Searching drag coefficient and timestep space for the convergence zone")}
  ...
  while((drag_iter <= hyper_iters) &
    ...
    #If the percentage change in res stat is smaller than the hyper tolerance the system is considered stable.
    ...
    if(verbose){
      ...
      print(paste("Hyper iteration",drag_iter, "drag value",
  ...
  if(min(memory_df$res_stat)>tol){
    ...
    embeddings_data <- setse_core_time_shift_hd(
      ...
      verbose = verbose) #This is false as something has to be produced by the algo. I can change later
