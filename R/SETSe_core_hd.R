...
SETSe_core_hd <- function(node_embeddings, 
  ...
  force <- NodeList %>% dplyr::select(starts_with("force_")) %>% as.matrix()
  elevation <- NodeList %>% dplyr::select(starts_with("elevation_")) %>% as.matrix()
  net_tension <-NodeList %>% dplyr::select(starts_with("net_tension_")) %>% as.matrix()
  velocity <- NodeList %>% dplyr::select(starts_with("velocity_")) %>% as.matrix()
  friction <- NodeList %>% dplyr::select(starts_with("friction_")) %>% as.matrix()
  static_force <-NodeList %>% dplyr::select(starts_with("static_force_")) %>% as.matrix()
  net_force <- NodeList %>% dplyr::select(starts_with("net_force_")) %>% as.matrix()
  acceleration <- NodeList %>% dplyr::select(starts_with("acceleration_")) %>% as.matrix()
